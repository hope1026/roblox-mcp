...
1. Read the governing UI/game spec, project UI manifest, token source, and approved asset manifest. Pass relevant evidence in `projectContext`; do not ask the Studio plugin to guess repository facts.
2. Start with `manage_ui.design_brief`. It may accept no brief, a partial brief, or a complete brief.
   Keep the accepted `brief_id`, `design_contract`, and advisory `layout_plan` through create, update, preview, and check. The contract uses `quality_priority=primary_view_first`, identifies the primary profile, and records whether responsive review is recommended or required. Treat region anchors and responsive behaviors as evidence-backed guidance, not a hard tree preset. For an existing target, follow the analysis-first redesign or targeted-update scope returned by `design_brief`.
